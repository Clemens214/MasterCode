...
numSize = 16;
stepSize = 20;
maxSize = minSize + (numSize-1)*stepSize;
sizesLead = linspace(minSize, maxSize, numSize);
sizesLead(1) = 1;
%% Calculate all the inputs
disp('Start preparing all the variables.')
SystemVals = cell(1, length(sizesLead));
angles = [0];
for i = 1:length(sizesLead)
    sizeLead = sizesLead(i);
    SystemVals{i} = setupSystem(sizeSample, orderSample, sizeLead, eigenenergy, hopping, hoppingsSample, hoppingLead, leadVals, derivVals, angles);
end
SystemVals = cell2mat(SystemVals);
chemPots = setupPots(voltages);
disp('Finished preparing all the variables.')

%% Compute the results
disp('Starting the calculation.')
Transmission = TransLin(SystemVals, chemPots, sizesLead, Energies);
...
%% plot the data
Plot(sizesLead, Energies, Transmission, conductance=true, Num=1, Title='Conductance', Names={'Transmission'})
...
function [SystemVals] = setupSystem(sizeSample, orderSample, sizeLead, eigenenergy, hopping, hoppingsSample, hoppingLead, leadVals, derivVals, angles)
    ...
    for i = 1:length(angles)
        ...
        [totalSystem, gammaL, gammaR] = makeSystemOld(sample, sizeSample, sizeLead, hoppingLead, hoppingsInter, leadVals);
        %[totalSystem, gammaL, gammaR] = makeSystemNew(sample, sizeSample, orderSample, sizeLead, hoppingLead, hoppingsInter, leadVals);
        % prepare the dreivative of the Hamiltonian
        sampleDeriv = zeros(length(sample), length(sample));
        hoppingDeriv = 0;
        [totalSysDeriv, ~, ~] = makeSystemNew(sampleDeriv, sizeSample, orderSample, sizeLead, ...
