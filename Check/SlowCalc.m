...
for i = 1:length(angleDiff)
    ...
    [totalSystem, gammaL, gammaR] = makeSystemNew(sample, sizeSample, orderSample, sizeLead, ...
    ...
    [totalSysDeriv, ~, ~] = makeSystemNew(sampleDeriv, sizeSample, orderSample, sizeLead, ...
                                        hoppingLeadDeriv, hoppingsDeriv, derivVals, check=false);
    ...
    for j = 1:length(voltages)
        ...
        [TotalTorque, LeftTorque, RightTorque] = torque(totalSystem, Eigenvals, leftEVs, rightEVs, totalSysDeriv, gammaL, gammaR, chemPots);
