function [ResultTotal, ResultLeft, ResultRight, varargout] = torqueCheck(totalSystem, Eigenvals, leftEVs, rightEVs, totalSysDeriv, gammaL, gammaR, chemPots, options)
    ...
    LeftTraces = allTrace(real(TorqueMatrices{1}));
    ResultLeft = LeftTraces(options.evalIndex);

    RightTraces = allTrace(real(TorqueMatrices{2}));
    ResultRight = RightTraces(options.evalIndex);
