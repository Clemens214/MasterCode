...
function [] = checkIndex(indicesL, indicesR, orderSample)
    if all(indicesL == indicesR)
        disp('The indices do match!')
        Varname = append('Indices', int2str(orderSample));
        assignin('base', Varname, indicesL);
    elseif ~all(indicesL == indicesR)
