...
function [Result, varargout] = ResultCalc(Matrices, Factors, options)
    ...
    for idx = 1:length(Matrices)
        if Matrices(idx).i == Factors(idx).i && Matrices(idx).j == Factors(idx).j
            Matrix = Matrix + Matrices(idx).matrix * Factors(idx).factor;
        else
            disp(['Indices do not match!', ...
                ' i=',num2str(Matrices(idx).i),', ',num2str(Factors(idx).i), ...
                ' j=',num2str(Matrices(idx).j),', ',num2str(Factors(idx).j)])
        end
    end
...
function [Matrices] = MatrixTorque(index, totalSysDeriv, midFactor)
    ...
    parfor idx = 1:numel(index)
        ...
        Matrices(idx).i = index(idx).i;
        Matrices(idx).j = index(idx).j;
    end
end
