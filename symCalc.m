...
hoppingsDerivAlt = [-1*sin(angle), cos(angle); -1*sin(angle), cos(angle)];
...
GreensLead = CalcGreens(Energy, hoppingLead, energyLead, eta=eta);
...
[spectralFuncL, GreensSample] = CalcSpectral(Energy, totalSystem, gammaL, gammaR, left=true);
spectralFuncR = CalcSpectral(Energy, totalSystem, gammaL, gammaR, right=true);
spectralFuncC = CalcSpectral(Energy, totalSystem, gammaL, gammaR, conservative=true);
spectralFuncNC = CalcSpectral(Energy, totalSystem, gammaL, gammaR, nonconservative=true);

[Transmission, TransMatrix, GreensFunc] = Calc(TransOp, spectralFuncL);
...
function [Result, varargout] = CalcSpectral (Energy, totalSystem, gammaL, gammaR, choice)
arguments
    Energy
    totalSystem
    gammaL
    ...
    % do the matrix multiplication
    Matrix = GreensSimp * midFactor * GreensSimp';
    MatrixSimp = simplify(Matrix);
    % return the results
    Result = MatrixSimp;
    varargout{1} = GreensSimp;
end
