...
TransmissionsOld = cell(length(angles), length(angles)); %= {cell([1, length(Temps)])};
TransmissionsNew = cell(length(angles), length(angles)); %= {cell([1, length(Temps)])};
for i = 1:length(angles)
    for j = 1:length(angles)
        ...
        TransmissionsOld{i, j} = zeros(1, length(Temps));
        TransmissionsNew{i, j} = zeros(1, length(Temps));
        ...
        for k = 1:length(Temps)
            ...
            [ResultOld] = transport(totalSystem, gammaL, gammaR, value, step);
            [ResultNew] = transmission(totalSystem, gammaL, gammaR);
            TransmissionsOld{i, j}(k) = ResultOld;
            TransmissionsNew{i, j}(k) = ResultNew;
...
figure(1)
hold on
anglePlot(TransmissionsOld, angleMax, angleNum)
hold off

figure(2)
hold on
anglePlot(TransmissionsNew, angleMax, angleNum)
hold off
